...
/// <summary>
/// Construye la pantalla "Elige tu contraseña" (paso 2 del registro).
/// Menú: ChemiTech → Build Register Password Scene
/// </summary>
public static class RegisterPasswordBuilder
{
    const float RW = 1600f, RH = 900f;
    const float PANEL_W = 760f, PANEL_H = 530f;

    [MenuItem("ChemiTech/Build Register Password Scene")]
    public static void Build()
    {
        if (!EditorUtility.DisplayDialog("Construir Register Password Scene",
            "Esto creará Assets/Scenes/RegisterPasswordScene.unity.\n¿Continuar?",
        ...
        // ── Puntos de progreso ────────────────────────────────────────────────
        // Dot1: verde (paso 1 completado), Dot2: cian (activo), Dot3: inactivo
        MakeDot(panelGo.transform, "Dot1", new Vector2(-22f, 228f), Hex("2ECC71"));
        MakeDot(panelGo.transform, "Dot2", new Vector2(  0f, 228f), Hex("4DD9E8"));
        MakeDot(panelGo.transform, "Dot3", new Vector2( 22f, 228f), new Color(1f, 1f, 1f, 0.3f));

        // ── Título ────────────────────────────────────────────────────────────
        var titleGo = MakeEmpty(panelGo.transform, "Title");
        SetRT(titleGo, new Vector2(0f, 168f), new Vector2(PANEL_W - 60f, 80f));
        var titleTmp = titleGo.AddComponent<TextMeshProUGUI>();
        titleTmp.text = "¡Elige tu contraseña!";
        titleTmp.font = fnt; titleTmp.fontSize = 54f; titleTmp.fontStyle = FontStyles.Bold;
        ...
        SetRT(subGo, new Vector2(0f, 112f), new Vector2(PANEL_W - 100f, 36f));
        var subTmp = subGo.AddComponent<TextMeshProUGUI>();
        subTmp.text = "Asegúrate de ingresar una contraseña segura";
        subTmp.font = fnt; subTmp.fontSize = 24f;
        subTmp.color = new Color(1f, 1f, 1f, 0.7f);
        subTmp.alignment = TextAlignmentOptions.Center; subTmp.enableWordWrapping = false;

        // ── Label campo ───────────────────────────────────────────────────────
        var labelGo = MakeEmpty(panelGo.transform, "LabelPassword");
        SetRT(labelGo, new Vector2(0f, 66f), new Vector2(PANEL_W - 100f, 28f));
        var labelTmp = labelGo.AddComponent<TextMeshProUGUI>();
        labelTmp.text = "Contraseña"; labelTmp.font = fnt; labelTmp.fontSize = 21f;
        labelTmp.color = Hex("4DD9E8"); labelTmp.alignment = TextAlignmentOptions.Left;
        labelTmp.overflowMode = TextOverflowModes.Overflow;

        // ── Input contraseña + toggle ojo ─────────────────────────────────────
        var inputField = MakePasswordInput(panelGo.transform, fnt, uiSpr, sprLock,
            "InputPassword", "••••••••", new Vector2(0f, 14f));

        // Toggle ojo (fuera del viewport, en el root del input)
        var inputGo = inputField.gameObject;
        const float INPUT_W = 580f;
        var toggleGo = MakeEmpty(inputGo.transform, "BtnToggle");
        SetRT(toggleGo, new Vector2(INPUT_W / 2f - 36f, 0f), new Vector2(32f, 32f));
        var toggleImg = toggleGo.AddComponent<Image>();
        toggleImg.color = new Color(1f, 1f, 1f, 0.55f);
        if (sprEye != null) { toggleImg.sprite = sprEye; toggleImg.preserveAspect = true; }
        var btnToggle = toggleGo.AddComponent<Button>();
        btnToggle.targetGraphic = toggleImg;

        // ── Barra de fortaleza ────────────────────────────────────────────────
        var barBgGo = MakeEmpty(panelGo.transform, "StrengthBarBg");
        SetRT(barBgGo, new Vector2(0f, -38f), new Vector2(INPUT_W, 8f));
        var barBgImg = barBgGo.AddComponent<Image>();
        barBgImg.color = new Color(1f, 1f, 1f, 0.15f);

        var barFillGo = MakeEmpty(barBgGo.transform, "Fill");
        var barFillRT = barFillGo.GetComponent<RectTransform>();
        barFillRT.anchorMin = new Vector2(0f, 0f); barFillRT.anchorMax = new Vector2(1f, 1f);
        barFillRT.offsetMin = barFillRT.offsetMax = Vector2.zero;
        var barFillImg = barFillGo.AddComponent<Image>();
        barFillImg.color      = Hex("E53535");
        ...
        // ── Label de fortaleza ────────────────────────────────────────────────
        var strengthLblGo = MakeEmpty(panelGo.transform, "StrengthLabel");
        SetRT(strengthLblGo, new Vector2(-INPUT_W / 2f + 2f, -58f), new Vector2(200f, 24f));
        var strengthLblTmp = strengthLblGo.AddComponent<TextMeshProUGUI>();
        strengthLblTmp.text = ""; strengthLblTmp.font = fnt; strengthLblTmp.fontSize = 20f;
        strengthLblTmp.color = Hex("E53535"); strengthLblTmp.alignment = TextAlignmentOptions.Left;
        strengthLblTmp.overflowMode = TextOverflowModes.Overflow;
        ...
        var (bgLen, txtLen) = MakeReqItem(panelGo.transform, fnt, roundedSpr, "ReqLength",
            "8+ caracteres",  new Vector2(col1X, -96f),  new Vector2(REQ_W, REQ_H));
        var (bgUpp, txtUpp) = MakeReqItem(panelGo.transform, fnt, roundedSpr, "ReqUpper",
            "Una mayúscula",  new Vector2(col2X, -96f),  new Vector2(REQ_W, REQ_H));
        var (bgNum, txtNum) = MakeReqItem(panelGo.transform, fnt, roundedSpr, "ReqNumber",
            "Un número",      new Vector2(col1X, -96f - REQ_H - REQ_GAP_Y), new Vector2(REQ_W, REQ_H));
        var (bgSym, txtSym) = MakeReqItem(panelGo.transform, fnt, roundedSpr, "ReqSymbol",
            "Un símbolo",     new Vector2(col2X, -96f - REQ_H - REQ_GAP_Y), new Vector2(REQ_W, REQ_H));
        ...
        // ── RegisterPasswordManager ───────────────────────────────────────────
        var mgrGo = MakeEmpty(cGo.transform, "RegisterPasswordManager");
        var mgr   = mgrGo.AddComponent<RegisterPasswordManager>();
        var so    = new SerializedObject(mgr);
        so.FindProperty("inputPassword").objectReferenceValue    = inputField;
        so.FindProperty("btnTogglePassword").objectReferenceValue = btnToggle;
        so.FindProperty("strengthBarFill").objectReferenceValue  = barFillImg;
        so.FindProperty("strengthLabel").objectReferenceValue    = strengthLblTmp;
        ...
        EditorSceneManager.SaveScene(scene, "Assets/Scenes/RegisterPasswordScene.unity");
        AssetDatabase.Refresh();

        Debug.Log("[RegisterPasswordBuilder] ✓ RegisterPasswordScene creada.");
        EditorUtility.DisplayDialog("¡Listo!",
            "RegisterPasswordScene creada.\n\n" +
            "Pasos finales:\n" +
    ...
    // ── Input password ────────────────────────────────────────────────────────
    static TMP_InputField MakePasswordInput(Transform parent, TMP_FontAsset fnt,
        Sprite uiSpr, Sprite iconSpr, string name, string placeholder, Vector2 pos)
    {
        const float W = 580f, H = 72f;
        ...
        areaRT.offsetMin = new Vector2(72f, 6f); areaRT.offsetMax = new Vector2(-52f, -6f);
        ...
        ph.text = placeholder; ph.font = fnt; ph.fontSize = 26f;
        ...
        txt.text = ""; txt.font = fnt; txt.fontSize = 26f; txt.color = Color.white;
        ...
        so.FindProperty("m_ContentType").enumValueIndex         = (int)TMP_InputField.ContentType.Password;
        ...
        Sprite rounded, string name, string label, Vector2 pos, Vector2 size)
    {
        ...
        txt.text = "○  " + label; txt.font = fnt; txt.fontSize = 20f;
    ...
    static void MakeDot(Transform parent, string name, Vector2 pos, Color color)
    {
        ...
        img.type = Image.Type.Simple; img.color = color;
    }
        ...
        string goName, string label, Vector2 pos, float size, Color color)
    {
        ...
        tmp.text = label; tmp.font = fnt; tmp.fontSize = size * 0.42f;
